<script setup>
import { ref, onUnmounted, onMounted, onBeforeMount, onBeforeUnmount  } from 'vue';
import { useRouter, onBeforeRouteLeave, onBeforeRouteUpdate } from 'vue-router'; // 导入 useRouter
...
const resizeSetter = () => {
  ...
  if (innerWidth >= 1200) { // 桌面端
    ...
    dropdownComponentWidth.value = '63%';
...
const handleSelectedItems = (item) => { 
  // 已选元素 => 全局状态 selectedItems 属性
  global.selectedItems = item; 
  // 已选元素（白字体） => 全局状态 selectedItems_white 属性
  global.selectedItems_white = global.selectedItems.map (item => {
        ...
        <div class="page page-direction">
          ...
          >
            <template #header-title>
              <!-- 流年运气求测须知 -->
              <img src="/assets/image/img/请选择您要求测的对象：.png" alt="图片失效">
            </template>
            <!-- 多选列表 -->
            <MultiSelectArray
              :requiredIcon="true"
