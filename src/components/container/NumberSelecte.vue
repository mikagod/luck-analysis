...
<script setup>
// 引入图片
import itemDefaul from '/assets/image/img/Property 1=Defaul.png'
import itemVariant from '/assets/image/img/Property 1=Variant.png'
// 导入模块
...
function confirmHandle () {
    ...
    if (selectedIndices.value.length !== 0) {
        ...
        console.log('确定')
        let num = selectedIndices.value[0]; // 数值
        let name = global.selectedItems_five[global.selectedItems_five.length - 1] // 名称
        global.selectedItems__Num.push({num, name}); // 添加数据    ***ADD***
        global.showNumSelecte = false // 关闭弹窗
        console.log('数组：', num,name,global.selectedItems__Num)
    }else {
        alert('请选择数字！')
...
<template>
    ...
    <div class="numSelectedLi">
        <img class="titleStyle" src="/assets/image/img/请选择：.png" alt="图片失效"> 
        ...
        <img class="btn" src="/assets/image/img/Btn确认.png" alt="图片失效" @click="confirmHandle">
    </div>
</template>
...
.numSelectedLi {
    ...
    align-items: center;
    /* 背景 */
    background-image: url("/assets/image/img/Bg弹窗.png");
